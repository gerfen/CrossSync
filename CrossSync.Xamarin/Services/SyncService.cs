...
namespace CrossSync.Xamarin.Services
{
    ...
    public abstract class SyncService<T> : IMobileSyncService<T>, ISyncService where T : class, IVersionableEntity, new()
    {
        ...
        protected SyncService(IUnitOfWork<IClientContext> uof, IConnectivityService connectivityService, Lazy<IErrorService> errorService, SyncConfiguration configuration)
        {
            this.uof = uof;
        ...
        public async Task SyncAsync()
        {
            ...
            //errorService.Value.ShowError($"Synching");

            //if (!Debugger.IsAttached && !connectivityService.IsConnected)
            //{
            //    errorService.Value.ShowError($"Device does not have connectivity.");
            //    return;
            //}
            ...
            try
            {
                ...
                })
                {
                    ...
                    foreach (var entity in entities)
                    {
                        ...
                        if (serverValue == null || serverValue.Version == entity.Version)
                        {
                            continue;
                        }
                        ...
                        {
                            var resolvedEntity = serverValue;
                            if (this is IConflictHandler<T> conflictHandler)
                            {
